...
let db = new sqlite3.Database('database.db', (err) => {
  if (!err) {
    ...
    db.run('CREATE TABLE IF NOT EXISTS tbl_my_resume (date DATE, title TEXT, content TEXT, url TEXT, UNIQUE(date, title))', (err2) => {
      if (!err2) {
        const resume = [
          {
            date: '1980-11-27',
          ...
          },
          {
            ...
            content: '당시 게임 시장에 대한 정확한 분석과 정보를 바탕으로 사람들의 게임에 대한 알권리를 제공하는 Play Ground Gameing Inc. 에 Journalist로 입사.',
            url: null,
          },
          {
            date: '2014-01-01',
            title: 'DOPT 생성',
            content: '사람들에게 더욱 더 많은 프로그램을 배포하고자 하는 목적으로 Dong Project Team이라는 팀을 생성.',
